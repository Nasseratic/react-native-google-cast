...
exports.MATCH_SWIFT_INIT = exports.MATCH_INIT = void 0;
...
const withIosAppDelegateLoaded = (config, props) => {
  return (0, _configPlugins.withAppDelegate)(config, config_ => {
    if (config_.modResults.language === 'objc' || config_.modResults.language === 'objcpp') {
      ...
      throw new Error('react-native-google-cast config plugin only supports Objective-C(++) or Swift AppDelegates.');
    }
    return config_;
  });
};
...
const MATCH_SWIFT_INIT = exports.MATCH_SWIFT_INIT = /public\s+override\s+func\s+application\s*\(\s*_?\s*application\s*:\s*UIApplication\s*,\s*didFinishLaunchingWithOptions\s+launchOptions\s*:.*\)\s*->\s*Bool/g;
